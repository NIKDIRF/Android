...
import com.app.contactlist.databinding.ItemBinding
import com.app.contactlist.databinding.ItemBlueBinding
import com.app.contactlist.model.Contact
import java.util.*


class RecycleViewAdapter(private var contacts: List<Contact>, val onClick: (Contact) -> Unit, val onClickSendMessage: (Contact) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        val holder = when (viewType) {
            3 -> {
                ContactBlueViewHolder(ItemBlueBinding.inflate(LayoutInflater.from(parent.context), parent, false)).apply {
                ...
                ContactViewHolder(ItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)).apply {
                    binding.root.setOnClickListener {
                        onClick(contacts[adapterPosition])
                    }
                    binding.sendMessage.setOnClickListener {
        ...
        if (charText.isNotEmpty()) {
            ...
            for (contact in contacts) {
                if (contact.name.toLowerCase(Locale.getDefault())
                    ...
                    contact.phoneNumber.toLowerCase(Locale.getDefault())
                        .contains(charText.toLowerCase(Locale.getDefault()))
                ) {
